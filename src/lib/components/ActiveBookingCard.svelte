...
<article class={`relative flex min-h-[15vw] flex-col justify-between overflow-hidden rounded-[2vw] border border-slate-800/80 p-[2.5vw] shadow-2xl shadow-black/40 ${bookingViewState === "idle" ? "border-emerald-500/20 bg-gradient-to-br from-emerald-950/40 via-black to-black" : "border-red-500/15 bg-gradient-to-br from-red-950/40 via-black to-black"}`}>
    ...
    <div>
        <div class={`inline-flex items-center gap-[0.5vw] rounded-full border px-[1.2vw] py-[0.4vw] text-[1.1vw] font-bold tracking-[0.12em] ${bookingViewState === "idle" ? "border-emerald-400/20 bg-emerald-500/15 text-emerald-300" : "border-red-400/20 bg-red-500/15 text-red-300"}`}>
            <span class={`h-[0.6vw] w-[0.6vw] min-h-1.5 min-w-1.5 animate-pulse rounded-full ${bookingViewState === "idle" ? "bg-emerald-400" : "bg-red-500"}`}></span>
    ...
    <div class="mt-[1vw] mb-[0.1vw]">
        <div class="mb-[0.2vw] flex items-center justify-between text-[1.2vw] text-slate-400">
            <span>{progressNote}</span>
            <span class="font-semibold text-orange-400">{progressPercent}%</span>
        </div>
        <div class="h-[0.3vw] min-h-[6px] w-full overflow-hidden rounded-full bg-slate-900">
            <div
