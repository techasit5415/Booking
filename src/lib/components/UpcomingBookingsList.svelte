...
<Card.Root
	class="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 rounded-xl flex flex-col gap-4 shadow-xs"
>
	<Card.Header class="p-0 flex flex-row items-center justify-between">
		<h3
			class="text-zinc-500 dark:text-zinc-400 text-xs font-bold tracking-wider uppercase flex items-center gap-1.5 font-['Prompt',sans-serif]"
		>
		...
		{#if upcomingBookings.length > 0}
			<Badge
				variant="secondary"
				class="bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-250/20 text-[10px] font-bold"
			>
				...
				{#each upcomingBookings as booking (booking.id)}
					...
					>
						<div class="min-w-0 flex-1 space-y-1">
							<p
								class="truncate text-sm font-bold text-zinc-800 dark:text-zinc-200 font-['Prompt',sans-serif] tracking-tight"
							>
								{booking.title}
							</p>
							<div
								class="flex flex-col sm:flex-row sm:items-center gap-x-4 gap-y-0.5 text-xs text-zinc-500"
							>
								{#if roomLocation}
